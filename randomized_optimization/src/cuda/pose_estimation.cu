    ...
    namespace internal {
        namespace cuda {
            ...
            void particle_kernel(const PtrStepSz<short> tsdf_volume,const PtrStep<float3> vertex_map_current,
                                 ...
                                 PtrStep<int> search_count, PtrStep<int> valid_pixels_count,
                ...
                {
                const int p = blockIdx.x * blockDim.x + threadIdx.x;  // particle index
                const int x = (blockIdx.y * blockDim.y + threadIdx.y)*level+level_index;  // x (pixel coordinate)
                const int y = (blockIdx.z * blockDim.z + threadIdx.z)*level+level_index;  // y (pixel coordinate)
                ...
                atomicAdd_system(valid_pixels_count+p,1);
            ...
            {       
                ...
                const int cols = vertex_map_current.cols;  // image width
                ...
                search_data.gpu_valid_pixels_count[particle_index/20].setTo(0);

                // measure geometric consistency error with TSDF
                particle_kernel<<<grid,block>>>(volume.tsdf_volume,vertex_map_current,normal_map_current,
                                        ...
                                        search_data.gpu_valid_pixels_count[particle_index/20],
                ...
                cv::Mat valid_pixels_count=search_data.valid_pixels_count[particle_index/20];
                ...
                search_data.gpu_valid_pixels_count[particle_index/20].download(valid_pixels_count);
                ...
                if (orgin_count==0){
                    orgin_tsdf = 1.0 / DIVSHORTMAX;
                }

                clock_t time_2=clock();
                
                int count_search=0.0;  // count particles (delta poses) that can improve original pose
                ...
                int sum_effective_pixels = 0;
                int sum_valid_pixels = 0;

                // count particles (delta poses) that can improve original pose
                for (int i=1; i<iter_rows ;++i){
                    ...
                    if ( tsdf_value<orgin_tsdf && ((search_data_count.ptr<int>(i)[0])>(orgin_count/2.0))){
                        sum_effective_pixels += search_data_count.ptr<int>(i)[0];
                        sum_valid_pixels += valid_pixels_count.ptr<int>(i)[0];
                        ...
                        const double weight=orgin_tsdf-tsdf_value;  // more tsdf reduction means better particle thus larger weight
