    ...
    namespace internal {
        namespace cuda {
                                           ...
                                           int* valid_pixels_count)
            {
                ...
                if (depth_value > 0.f) {
                    atomicAdd_system(valid_pixels_count, 1);
                }
                                    ...
                                    const CameraParameters cam_params, GpuMat& valid_pixels_count)
            {
                ...
                kernel_compute_vertex_map << < blocks, threads >> > (depth_map, vertex_map, depth_cutoff, cam_params, valid_pixels_count.ptr<int>(0));
